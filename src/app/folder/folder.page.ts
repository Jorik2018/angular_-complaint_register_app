  ...
  uploadFilesSimulator(index: number) {
	...
	*/
	...
	async send(){
		...
		loading.present();
		console.log(me.o);
		me.http.post<any>('http://localhost:8100/folder/Favorites', me.o).subscribe(data => {
			console.log(data);
		},(error)=>{
			console.log(error);
			setTimeout(()=>{
	...
	upload(file:any,callback?:any){
		...
		setTimeout(async()=>{
			...
			loading.present()
			me.http.post(me.baseAPI+'/api/file/upload',formData,{observe:'events',
			reportProgress: true}).subscribe((event:any) => {
			switch (event!.type){
				case HttpEventType.UploadProgress:
					if(event!.total){
